...
class TcpConnectionHandler extends handler_1.ConnectionHandler {
    constructor(context, settings, statusBar, logger, config) {
        ...
        if (this.connectionType === interfaces_1.ConnectionType.Local) {
            let exe = commandHelper_1.CommandEnvironmentHelper.getRubyEnvFromConfiguration(this.context.asAbsolutePath(this.config.languageServerPath), this.settings, this.config);
            let logPrefix = '';
