...
/**
 * Parser that formats a W3C extended log file for processing as a CSV
 *
 * See https://www.w3.org/TR/WD-logfile.html
 */
class W3CFileParser {
    /**
     * Return text ready to be formatted by CsvParser
     * @param text The W3C log file data to parse
     */
    parse(text) {
        let result = '';
        const lines = text.split("\n");
        const FieldsPrefix = '#Fields:';
        // Identify what our desired separator should be
        // Process each line in sequence
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
            // Convert this line to SSV if 
            // If this is 
        }
        return result;
    }
    ...
    getRecords() {
        let records = [];
        let currentRecord = new CsvRecord();
...
//# sourceMappingURL=W3CFileParser.js.map