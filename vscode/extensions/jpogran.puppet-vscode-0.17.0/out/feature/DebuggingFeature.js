...
class DebugAdapterDescriptorFactory {
    constructor(context, settings, config, logger) {
        this.ChildProcesses = [];
    ...
    createDebugAdapterDescriptor(session, executable) {
        ...
        return new Promise(function (resolve, reject) {
            let debugServer = commandHelper_1.CommandEnvironmentHelper.getDebugServerRubyEnvFromConfiguration(thisFactory.Context.asAbsolutePath(thisFactory.Config.debugServerPath), thisFactory.Settings, thisFactory.Config);
            let spawn_options = {};
...
class DebuggingFeature {
    constructor(debugType, settings, config, context, logger) {
